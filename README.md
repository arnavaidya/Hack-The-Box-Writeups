# Hack The Box Writeups

A collection of my Hack The Box (HTB) machine writeups, walkthroughs, and notes created for learning, revision, and knowledge sharing.

## Disclaimer

These writeups are intended for educational purposes only and are based on retired Hack The Box machines, Starting Point machines, or lab environments specifically designed for cybersecurity training.

All techniques demonstrated were performed in authorized environments. Do not attempt to use these techniques against systems without explicit permission.

---

## Starting Point

### Tier 0 - Foundations

| Machine  | Difficulty | Writeup                                                         |
| -------- | ---------- | --------------------------------------------------------------- |
| Meow     | Very Easy  | [View](Starting%20Point/Tier%200%20-%20Foundations/Meow.md)     |
| Fawn     | Very Easy  | [View](Starting%20Point/Tier%200%20-%20Foundations/Fawn.md)     |
| Dancing  | Very Easy  | [View](Starting%20Point/Tier%200%20-%20Foundations/Dancing.md)  |
| Redeemer | Very Easy  | [View](Starting%20Point/Tier%200%20-%20Foundations/Redeemer.md) |

---

### Tier 1 - Fundamental Exploitation

| Machine     | Difficulty | Writeup                                                                           |
| ----------- | ---------- | --------------------------------------------------------------------------------- |
| Appointment | Very Easy  | [View](Starting%20Point/Tier%201%20-%20Fundamental%20Exploitation/Appointment.md) |
| Crocodile   | Very Easy  | [View](Starting%20Point/Tier%201%20-%20Fundamental%20Exploitation/Crocodile.md)   |
| Responder   | Very Easy  | [View](Starting%20Point/Tier%201%20-%20Fundamental%20Exploitation/Responder.md)   |
| Sequel      | Very Easy  | [View](Starting%20Point/Tier%201%20-%20Fundamental%20Exploitation/Sequel.md)      |
| Three       | Very Easy  | [View](Starting%20Point/Tier%201%20-%20Fundamental%20Exploitation/Three.md)       |


---

### Tier 2 - Multi-Step Attacks and Privilege Escalation

| Machine     | Difficulty | Writeup                                                                                      |
| ----------- | ---------- | -------------------------------------------------------------------------------------------- |
| Vaccine | Very Easy | [View](Starting%20Point/Tier%202%20-%20Multi-Step%20Attacks%20and%20Privilege%20Escalation/Vaccine.md) |
| Oopsie | Very Easy | [View](Starting%20Point/Tier%202%20-%20Multi-Step%20Attacks%20and%20Privilege%20Escalation/Oopsie.md) |
| Archetype | Very Easy | [View](Starting%20Point/Tier%202%20-%20Multi-Step%20Attacks%20and%20Privilege%20Escalation/Archetype.md) |
| Unified | Very Easy | [View](Starting%20Point/Tier%202%20-%20Multi-Step%20Attacks%20and%20Privilege%20Escalation/Unified.md) |

---

## Machines

| Machine     | Difficulty | Writeup                                                                                      |
| ----------- | ---------- | -------------------------------------------------------------------------------------------- |
| Connected | Easy | [View](Connected.md) |
| Paperwork | Easy | [View](Paperwork.md) |
| Management | Easy | [View](Management.md) |


## Goals

* Document learning progress through HTB.
* Build a structured knowledge base for penetration testing techniques.
* Create concise and reproducible walkthroughs.
* Maintain notes for future OSCP, CPTS, and real-world engagements.

---
